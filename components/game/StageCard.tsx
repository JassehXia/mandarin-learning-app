    ...
    return (
        <div className={`group bg-white border ${isCompleted ? "border-green-500/30" : "border-[#E8E1D5]"} rounded-2xl p-6 hover:shadow-xl hover:border-[#D4AF37] transition-all duration-300 flex flex-col relative`}>
            ...
            {scenario.keyPhrases && Array.isArray(scenario.keyPhrases) && scenario.keyPhrases.length > 0 && (
                <div className="mb-8 pt-4 border-t border-[#E8E1D5] border-dashed">
                    <h3 className="text-[10px] font-bold text-[#D4AF37] uppercase tracking-[0.2em] mb-3">Helpful Phrases</h3>
                    <div className="flex flex-wrap gap-2">
                        {scenario.keyPhrases.slice(0, 3).map((kp: any, idx: number) => (
                            <div key={idx} className="bg-[#FDFBF7] border border-[#E8E1D5] rounded-lg px-2 py-1.5 transition-colors group-hover:bg-white">
                                <span className="text-xs font-bold text-[#2C2C2C] block">{kp.phrase}</span>
                                <span className="text-[10px] text-gray-400 italic block leading-tight">{kp.translation}</span>
                            </div>
                        ))}
                    </div>
                </div>
